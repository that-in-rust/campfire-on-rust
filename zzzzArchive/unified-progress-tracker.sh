        ...
        find "$GIT_ROOT/.kiro/specs" -mindepth 1 -maxdepth 1 -type d | sort | while read -r spec_dir; do
            ...
            if [ ! -f "$spec_dir/requirements.md" ]; then
                ...
                if [ -f "$spec_dir/tasks.md" ]; then
                    total_tasks=$(grep -c "^- \[" "$spec_dir/tasks.md" 2>/dev/null | head -1 || echo "0")
                    completed_tasks=$(grep -c "^- \[x\]" "$spec_dir/tasks.md" 2>/dev/null | head -1 || echo "0")
...
echo "This gladly works - am surprised"
